...
class IMUComplementaryFilter:
    ...
    def update(self, gyro, accel, dt):
        ...
        if accel_norm > 0.1:  # Avoid division by zero
            accel = accel / accel_norm * 9.81
            
            # Accelerometer-based attitude (roll and pitch only)
            ...
            # Complementary fusion - only for roll and pitch
            ...
            # Yaw accumulates from gyro only
        else:
            # Only integrate gyroscope if accelerometer is unreliable
            self.attitude += gyro * dt
        
        return self.attitude.copy()
    
    def get_rotation_matrix(self):
        """Get rotation matrix from current attitude"""
        return Rotation.from_euler('xyz', self.attitude).as_matrix()
    
    def get_gravity_aligned_transform(self):
        """Get 4x4 transformation matrix aligned with gravity"""
...
class IMUDataBuffer:
    def __init__(self, max_size=1000, max_time_diff=0.05):
        self.buffer = deque(maxlen=max_size)
        self.max_time_diff = max_time_diff
        
    def add_data(self, accel, gyro, timestamp):
        self.buffer.append({
            'accel': accel.copy(),
            'gyro': gyro.copy(), 
            'timestamp': timestamp
    ...
    def get_interpolated_data(self, target_time):
        """Get IMU data interpolated to target timestamp"""
        if len(self.buffer) < 2:
            return None if len(self.buffer) == 0 else self.buffer[-1]
        ...
        for data in self.buffer:
            time_diff = data['timestamp'] - target_time
            if time_diff <= 0:  # Before or at target time
                before = data
            elif time_diff > 0 and after is None:  # After target time
                after = data
                break
        
        if before is None:
            return after
        if after is None or abs(before['timestamp'] - target_time) > self.max_time_diff:
            return before
        ...
        if dt > 0:
            ...
            return {
                'accel': before['accel'] * (1 - alpha) + after['accel'] * alpha,
                'gyro': before['gyro'] * (1 - alpha) + after['gyro'] * alpha,
...
class RealSenseSlam3DScanner:
    def __init__(self, simple_mode=False):
        ...
        self.has_imu = self._check_imu_availability(device)
        if self.has_imu:
            print("IMU detected - enabling gravity-aligned reconstruction")
        else:
            print("No IMU detected - using standard reconstruction")
        ...
        if self.has_imu:
            try:
                ...
                print("IMU streams configured: Accel 63Hz, Gyro 200Hz")
        ...
        if self.has_imu:
            self.imu_filter = IMUComplementaryFilter(alpha=0.96)
            self.imu_buffer = IMUDataBuffer()
            self.last_imu_time = None
            self.pose_history = []  # Store poses for each frame
    ...
    def _check_imu_availability(self, device):
    ...
    def process_imu_data(self, frames):
        """Extract and process IMU data from frames"""
        if not self.has_imu:
            return None
        ...
        try:
            ...
            if accel_frame and gyro_frame:
                ...
                self.imu_buffer.add_data(accel, gyro, timestamp)
                
                # Update filter if we have timing
                if self.last_imu_time is not None:
                    dt = current_time - self.last_imu_time
                    if 0.001 < dt < 0.1:  # Reasonable time step
                        attitude = self.imu_filter.update(gyro, accel, dt)
    ...
    def get_imu_aligned_pose(self, frame_timestamp):
        """Get pose aligned with gravity using IMU data"""
        if not self.has_imu:
            return np.eye(4)
        
        # Get interpolated IMU data for frame timestamp
        imu_data = self.imu_buffer.get_interpolated_data(frame_timestamp / 1000.0)
        if imu_data is None:
            return np.eye(4)
        
        # Get gravity-aligned transformation
        gravity_transform = self.imu_filter.get_gravity_aligned_transform()
        
        # For simple mode, just return gravity alignment
        # For SLAM mode, this would be combined with visual odometry
        return gravity_transform
    
    def estimate_pose(self, current_rgbd, prev_rgbd, prev_pose, frame_timestamp=None):
        """Estimate camera pose using visual odometry with IMU assistance"""
        if prev_rgbd is None:
            return np.eye(4)
        
        intrinsic = self.get_camera_intrinsics()
        
        # Try visual odometry first
        visual_pose = self._visual_odometry(current_rgbd, prev_rgbd, prev_pose, intrinsic)
        
        # If IMU is available, use it to improve/constrain the pose
        if self.has_imu and frame_timestamp is not None:
            imu_pose = self.get_imu_aligned_pose(frame_timestamp)
            
            # Simple fusion: use IMU for orientation, visual for translation
            # Extract rotation from IMU, translation from visual
            visual_translation = visual_pose[:3, 3]
            imu_rotation = imu_pose[:3, :3]
            
            # Combine them
            fused_pose = np.eye(4)
            fused_pose[:3, :3] = imu_rotation
            fused_pose[:3, 3] = visual_translation
            
            return fused_pose
        
        return visual_pose
    
    def _visual_odometry(self, current_rgbd, prev_rgbd, prev_pose, intrinsic):
        """Perform visual odometry between consecutive frames"""
        try:
    ...
    def start_recording(self):
        ...
        mode_str = 'simple' if self.simple_mode else 'SLAM'
        imu_str = ' with IMU' if self.has_imu else ''
        print(f"Starting {mode_str} recording{imu_str}...")
        ...
        try:
            ...
            if self.state == ScanState.RECORDING:
                try:
                    if self.simple_mode:
                        # Simple mode: just accumulate point clouds
                        self._process_frame_simple(color_image, depth_image, frame_timestamp)
                    else:
                        # SLAM mode: full pose tracking
                        self._process_frame_slam(color_image, depth_image, frame_timestamp)
    ...
    def _process_frame_simple(self, color_image, depth_image, frame_timestamp):
        """Process frame in simple mode with IMU alignment"""
        if self.frame_count % 5 == 0:  # Every 5th frame to avoid too much data
            ...
            if len(current_pc.points) > 0:
                # Apply IMU-based gravity alignment if available
                if self.has_imu:
                    imu_transform = self.get_imu_aligned_pose(frame_timestamp)
                    current_pc.transform(imu_transform)
    ...
    def _process_frame_slam(self, color_image, depth_image, frame_timestamp):
        """Process frame in SLAM mode with IMU-enhanced pose estimation"""
        # Create RGBD image
        rgbd = self.create_rgbd_image(color_image, depth_image)
        
        # Estimate pose using visual odometry with IMU assistance
        self.current_pose = self.estimate_pose(rgbd, self.prev_rgbd, self.current_pose, frame_timestamp)
    ...
    def run_interface(self):
        ...
        if self.has_imu:
            print("✓ IMU-Enhanced Mode Active")
        print("Controls:")
        ...
        try:
            while True:
                ...
                if color_image is not None:
                    ...
                    status_text = f"Status: {self.state.value}"
                    if self.has_imu:
                        status_text += " (IMU)"
    ...
    try:
        ...
        for i, device in enumerate(devices):
            ...
            for j, sensor in enumerate(sensors):
                ...
                accel_streams = [p for p in profiles if p.stream_type() == rs.stream.accel]
                gyro_streams = [p for p in profiles if p.stream_type() == rs.stream.gyro]
                ...
                if accel_streams:
                    print(f"        Accelerometer streams: {len(accel_streams)}")
                    has_imu = True
                    for p in accel_streams[:2]:
                        print(f"          {p.format()} @ {p.fps()}fps")
                
                if gyro_streams:
                    print(f"        Gyroscope streams: {len(gyro_streams)}")
                    has_imu = True
                    for p in gyro_streams[:2]:
                        print(f"          {p.format()} @ {p.fps()}fps")
            
            if has_imu:
                print("    ✓ IMU sensors detected - gravity alignment available!")
            else:
                print("    ⚠ No IMU sensors detected - using standard reconstruction")
